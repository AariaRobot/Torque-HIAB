...
model = 'AariaHIABTorqueR2017a';
simIn(1) = Simulink.SimulationInput(model);
%open('AariaJoystickR2017a.slx')
set_param('AariaHIABTorqueR2017a','SimulationMode','normal')
set_param('AariaHIABTorqueR2017a','SimMechanicsOpenEditorOnUpdate','on')
set_param('AariaHIABTorqueR2017a','StopTime','inf')
...
for nLL =1:AARIAMAN.linknumber-1
    ...
    AARIAMAN.L(nLL).rod =  [AARIAMAN.vis.link_width_x/2 AARIAMAN.vis.link_width_y/2 abs(AARIAMAN.DH(nLL).d) + 0.001]; %Prismatic link dimensions [m]
...
AARIAMAN.D.revolutedamping = 10; %Internal joint damping coefficient N*m/(deg/s)
...
Camera.ObjectTranslation = [vec_rand_ab_res([-3,3,0.1]) vec_rand_ab_res([-3,3,0.1]) vec_rand_ab_res([0.1,3,0.1])]; % Cartesian object translation
